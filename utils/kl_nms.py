...
def soft_nms(dets, box_scores, iou_thresh=None, method="Gaussian", sigma=0.5, conf_thres=0.2):
    """
    ...
    dets = torch.cat((dets, indexes), dim=1).cuda()
    ...
    for i in range(N):
        ...
        # elif method == "Greedy":
        #     zero = torch.zeros_like(ious)
        #     box_scores[pos:] = torch.where(ious < iou_thresh, zero, ious)
    ...
    keep = dets[:, 4][box_scores > conf_thres].long()
